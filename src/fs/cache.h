...
typedef struct block {
     ...
     *
     * You may find it useless and it *is*. It is just a test flag read by our
     * test. In your code, you should:
...
typedef struct block_cache {
     ...
     * block so that the caller can exclusively modify it. It returns the
     * pointer to the locked block.
     */
     ...
     *
     * If there are too many running operations (i.e. our logging is too small
     * to hold all of them), `begin_op` should sleep until we can start a new
     * operation.
     *
     ...
     *
     * It will throw panic if the number of blocks associated with `ctx` is
     * larger than `OP_MAX_NUM_BLOCKS` after `sync`
     */
