...
EMMCCommand sd_command_table[] = {
	...
	{ "SD_STATUS", 0x0D000000 | CMD_RSPNS_48, RESP_R1, RCA_YES,
	  0 }, // RCA???
...
static int sd_send_commandP(EMMCCommand *cmd, int arg)
{
	...
	*EMMC_CMDTM = cmd->code;
	...
	case RESP_R6:
		...
		sdd.status =
			(u32)((resp0 &
			       0x00001fff)) // 12:0 map directly to status 12:0
			| (u32)((resp0 & 0x00002000)
				<< 6) // 13 maps to status 19 ERROR
...
static int sd_reset_card(int resetType)
{
	...
	*EMMC_IRPT_EN = 0xffffffff & (u32)(~INT_CMD_DONE) &
			(~(u32)INT_WRITE_RDY);
...
void sd_parse_csd()
{
	...
	if (ccd_version == CSD0_V1) {
		int csize = (int)(((sdd.cSD[1] & CSD1V1_C_SIZEH)
				   << CSD1V1_C_SIZEH_SHIFT) +
				  ((sdd.cSD[2] & CSD2V1_C_SIZEL) >>
				   CSD2V1_C_SIZEL_SHIFT));
...
int sd_init()
{
	...
	sd_host_version = (*EMMC_SLOTISR_VER & HOST_SPEC_NUM) >>
			  HOST_SPEC_NUM_SHIFT;
	...
	if (sdd.support & SD_SUPP_BUS_WIDTH_4) {
		if ((resp = sd_send_command_arg(IX_SET_BUS_WIDTH,
						(int)sdd.rca | 2)))
			return sd_debug_response(resp);
