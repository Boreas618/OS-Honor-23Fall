...
static void __rb_insert_fix(rb_node node, rb_root root)
{
	...
	while (1) {
		...
		if (parent != tmp) { /* parent == gparent->rb_left */
			...
			tmp = parent->rb_left;
			if (node ==
			    tmp) { /*Case 2,node is right child,left rotate*/
	...
	while (1) {
		...
		if (node != sibling) {
			...
			if (!tmp1 || rb_is_black(tmp1)) {
				...
				if (!tmp2 ||
				    rb_is_black(
					    tmp2)) { /*Case 2,sibling black,ch1,ch2 black*/
...
{
	acquire_spinlock(&rbtree->rblock);
}
...
int rbtree_insert(RBTree *rbtree, rb_node node,
		  bool (*cmp)(rb_node lnode, rb_node rnode))
{
	int r = 0;
...
rb_node rbtree_lookup(RBTree *rbtree, rb_node node,
		      bool (*cmp)(rb_node lnode, rb_node rnode))
{
	rb_node r = NULL;
