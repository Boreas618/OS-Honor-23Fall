...
void set_user_init() {
    ...
    create_vmregion(&p->vmspace, ST_TEXT, 0x8000, (u64)eicode - PAGE_BASE((u64)icode));

    for (u64 i = PAGE_BASE((u64)(icode)); i <= (u64)eicode; i += PAGE_SIZE)
        map_in_pgtbl(p->vmspace.pgtbl, 0x8000 + (i - PAGE_BASE((u64)icode)),
                     (void *)i, PTE_USER_DATA | PTE_RO);
