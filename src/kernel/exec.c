...
static int execve_load_section(struct vmspace *vms, Elf64_Phdr ph,
    ...
    struct vmregion *vmr = create_vmregion(vms, flags, ph.p_vaddr, ph.p_memsz);
    ...
    while (begin < end) {
        ...
        u64 len = MIN(end - begin, (u64)PAGE_SIZE - (begin - PAGE_BASE(begin)));
        u8 *dest = (u8 *)((u64)page_to_map + (begin - PAGE_BASE(begin)));
        ...
        map_in_pgtbl(vms->pgtbl, PAGE_BASE(begin), page_to_map, pte_flags);
